...
export async function POST(req: Request) {
  ...
  console.log("=== Shelf API POST called ===");
  console.log("Shelf API: Raw request body client_title:", json?.client_title?.substring(0, 100));
  ...
  console.log("Shelf API: client_title length:", json?.client_title?.length || 0);
  console.log("Shelf API: Full request body keys:", Object.keys(json || {}));
  console.log("Shelf API: Full request body:", JSON.stringify(json, null, 2).substring(0, 500));
  ...
  if (!isTextNote) {
    ...
    const bookmarkType = detectBookmarkType(normalized || input || urlToStore, isTextNote);
    isX = bookmarkType === "x";
  ...
  console.log("Shelf API: Final X bookmark detection result", { isX, isTextNote, normalized, input: input?.substring(0, 100) });
  ...
  const isValidClientTitle = clientTitle && clientTitle.trim() && clientTitle.trim() !== "X";
  ...
  if (isTextNote) {
    ...
    console.log("Shelf API: [TITLE DEBUG] Text note - using titleToStore:", finalTitle);
    ...
    console.log("Shelf API: [TITLE DEBUG] Using clientTitle (highest priority):", finalTitle.substring(0, 150));
    ...
    console.log("Shelf API: [TITLE DEBUG] finalTitle length:", finalTitle.length);
  } else if (isX) {
    // For X bookmarks without clientTitle, try to fetch server-side
    console.log("Shelf API: [TITLE DEBUG] X bookmark without clientTitle, fetching server-side");
    let fetchedTitle: string | null = null;
    if (normalized) {
      console.log("Shelf API: [TITLE DEBUG] Calling fetchXTitle with:", normalized);
    ...
    if (fetchedTitle && fetchedTitle.trim() && fetchedTitle.trim() !== "X") {
      ...
      console.log("Shelf API: [TITLE DEBUG] Using metadata title as fallback:", finalTitle?.substring(0, 150));
    }
  } else if (shouldPreferClientTitleForLinkedIn && clientTitle) {
    finalTitle = clientTitle;
    console.log("Shelf API: [TITLE DEBUG] LinkedIn - using clientTitle:", finalTitle?.substring(0, 150));
  } else {
    // Default: use metadata title, fall back to clientTitle if available
    finalTitle = metadata?.title ?? null;
    console.log("Shelf API: [TITLE DEBUG] Default - using metadata title:", finalTitle?.substring(0, 150));
    
    if (!finalTitle && isValidClientTitle) {
      finalTitle = clientTitle.trim();
      console.log("Shelf API: [TITLE DEBUG] No metadata title, using clientTitle:", finalTitle?.substring(0, 150));
    }
  }
  
  console.log("Shelf API: [TITLE DEBUG] finalTitle after determination:", finalTitle?.substring(0, 150));
  ...
  if (isValidClientTitle) {
    ...
    console.log("Shelf API: [TITLE DEBUG] Using clientTitle directly (highest priority):", titleToInsert.substring(0, 150));
  } 
  // PRIORITY 2: Use finalTitle if it's valid
  else if (finalTitle && finalTitle.trim() && finalTitle.trim() !== "X") {
    titleToInsert = finalTitle.trim();
    console.log("Shelf API: [TITLE DEBUG] Using finalTitle:", titleToInsert.substring(0, 150));
  }
  ...
  if (isX && !titleToInsert) {
    console.error("Shelf API: [TITLE DEBUG] WARNING - X bookmark has no valid title, using URL as fallback", {
      clientTitle: clientTitle?.substring(0, 150),
      finalTitle: finalTitle?.substring(0, 150),
    ...
    if (urlToParse) {
      try {
        ...
        if (pathParts.length >= 1 && pathParts[0] && pathParts[0] !== 'home' && pathParts[0] !== 'explore') {
          titleToInsert = `Post by @${pathParts[0]}`;
          console.log("Shelf API: [TITLE DEBUG] Generated title from URL path:", titleToInsert);
        } else {
          titleToInsert = "X post";
          console.log("Shelf API: [TITLE DEBUG] Using generic X post title");
        }
      } catch (e) {
        console.error("Shelf API: [TITLE DEBUG] Failed to parse URL for fallback title", e);
        titleToInsert = "X post";
      }
    } else {
      titleToInsert = "X post";
      console.log("Shelf API: [TITLE DEBUG] No URL available, using generic title");
    }
  }

  // CRITICAL FINAL CHECK: If we still don't have a title but have clientTitle, use it
  if (!titleToInsert && isValidClientTitle) {
    console.error("Shelf API: [TITLE DEBUG] CRITICAL - titleToInsert is null but we have valid clientTitle! Using it now.");
    titleToInsert = clientTitle.trim();
  }
  ...
  if (!titleToInsert && clientTitle && clientTitle.trim() && clientTitle.trim() !== "X") {
    console.error("Shelf API: [TITLE DEBUG] ABSOLUTE FINAL SAFETY - Setting titleToInsert from clientTitle");
    titleToInsert = clientTitle.trim();
  }
  ...
  console.log("Shelf API: Full responseData:", JSON.stringify(responseData, null, 2).substring(0, 2000));
  ...
  console.log("Shelf API: [TITLE DEBUG] Response created, status:", jsonResponse.status);
